...
class StatsCalculatorTest {
    ...
    @Test
    fun `vitality - zero weigh-in days returns 1`() {
        val stat = StatsCalculator.calculateVitality(weighInDays = 0, bodyFatTrendPercent = null)
        assertEquals(1, stat)
    }
    ...
    fun `vitality - 1 weigh-in day with no trend data yields low stat`() {
        ...
        // blend: (1 + 50) / 2 ≈ 26
        val stat = StatsCalculator.calculateVitality(weighInDays = 1, bodyFatTrendPercent = null)
    ...
    fun `vitality - 7 weigh-in days yields frequency around 25`() {
        ...
        // blend: (25 + 50) / 2 ≈ 38
        val stat = StatsCalculator.calculateVitality(weighInDays = 7, bodyFatTrendPercent = null)
    ...
    fun `vitality - 14 weigh-in days yields frequency around 50`() {
        ...
        // blend: (50 + 50) / 2 = 50
        val stat = StatsCalculator.calculateVitality(weighInDays = 14, bodyFatTrendPercent = null)
    ...
    fun `vitality - 21 weigh-in days yields frequency around 75`() {
        ...
        // blend: (75 + 50) / 2 ≈ 63
        val stat = StatsCalculator.calculateVitality(weighInDays = 21, bodyFatTrendPercent = null)
    ...
    fun `vitality - 28 weigh-in days yields frequency around 99`() {
        ...
        // blend: (99 + 50) / 2 ≈ 75
        val stat = StatsCalculator.calculateVitality(weighInDays = 28, bodyFatTrendPercent = null)
    ...
    fun `vitality - improving body fat trend yields high trend component`() {
        ...
        // blend: (50 + 99) / 2 ≈ 75
        val stat = StatsCalculator.calculateVitality(weighInDays = 14, bodyFatTrendPercent = -1.5)
    ...
    fun `vitality - stable body fat yields moderate-high trend`() {
        ...
        // blend: (50 + 75) / 2 ≈ 63
        val stat = StatsCalculator.calculateVitality(weighInDays = 14, bodyFatTrendPercent = 0.0)
        assertTrue("Stable trend should yield ~63, got $stat", stat in 55..70)
    }

    @Test
    fun `vitality - slight gain in body fat yields moderate trend`() {
        // frequency: 14 → 50, trend: 1.25% gain → 50
        // blend: (50 + 50) / 2 = 50
        val stat = StatsCalculator.calculateVitality(weighInDays = 14, bodyFatTrendPercent = 1.25)
        assertTrue("Slight gain should yield ~50, got $stat", stat in 40..55)
    }
    ...
    fun `vitality - moderate gain in body fat yields low trend`() {
        ...
        // blend: (50 + 25) / 2 ≈ 38
        val stat = StatsCalculator.calculateVitality(weighInDays = 14, bodyFatTrendPercent = 3.0)
    ...
    fun `vitality - significant gain in body fat yields minimum trend`() {
        ...
        // blend: (50 + 1) / 2 ≈ 26
        val stat = StatsCalculator.calculateVitality(weighInDays = 14, bodyFatTrendPercent = 5.0)
    ...
    fun `vitality - null body fat trend defaults to 50 for trend component`() {
        ...
        val statWithNull = StatsCalculator.calculateVitality(weighInDays = 14, bodyFatTrendPercent = null)
        val statWithStable = StatsCalculator.calculateVitality(weighInDays = 14, bodyFatTrendPercent = 0.0)
    ...
    fun `vitality - perfect consistency with improving trend yields near 99`() {
        ...
        // blend: (99 + 99) / 2 = 99
        val stat = StatsCalculator.calculateVitality(weighInDays = 28, bodyFatTrendPercent = -2.0)
        assertTrue("Perfect consistency + improving should be near 99, got $stat", stat >= 90)
    }

    @Test
    fun `vitality - zero data returns minimum stat`() {
        // frequency: 0 → below brackets → 1, trend: null → 50
        // blend: (1 + 50) / 2 but if weighInDays=0, total should be 1
        val stat = StatsCalculator.calculateVitality(weighInDays = 0, bodyFatTrendPercent = null)
        assertEquals("Zero data should yield 1", 1, stat)
    }

    @Test
    fun `vitality - stat clamped to 1-99 range`() {
        // Test extreme values don't exceed range
        val high = StatsCalculator.calculateVitality(weighInDays = 30, bodyFatTrendPercent = -5.0)
        val low = StatsCalculator.calculateVitality(weighInDays = 0, bodyFatTrendPercent = 10.0)
    ...
    fun `vitality - 50-50 blend verified with known inputs`() {
        ...
        // expected: (99 + 75) / 2 ≈ 87
        val stat = StatsCalculator.calculateVitality(weighInDays = 28, bodyFatTrendPercent = 0.0)
    ...
    fun `vitalityBreakdown includes weigh-in count detail`() {
        val breakdown = StatsCalculator.vitalityBreakdown(weighInDays = 14, bodyFatTrendPercent = -0.5)
        val keys = breakdown.details.map { it.first }
    ...
    fun `vitalityBreakdown shows no trend data message when null`() {
        val breakdown = StatsCalculator.vitalityBreakdown(weighInDays = 14, bodyFatTrendPercent = null)
    ...
    fun `vitalityBreakdown brackets string is not empty`() {
        val breakdown = StatsCalculator.vitalityBreakdown(weighInDays = 14, bodyFatTrendPercent = 0.0)
